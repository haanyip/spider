...
def crawl(conn):
    ...
    for tag in tags:
        ...
        content = str(content)
        start = content.find('<script>var props=')
        end = content.find('</script>', start)
        jStr = content[start + 18:end]
        start = jStr.find('"content":"')
        end = jStr.find('"cover"', start)
        html = jStr[start + 11:end - 2]
        html = cgi.escape(html, True)
        # print html
